...
Original file is located at
    https://colab.research.google.com/#fileId=https%3A//storage.googleapis.com/kaggle-colab-exported-notebooks/wastesortai-pcode-2f3571a0-259e-43b4-b5e9-f1d57656429f.ipynb%3FX-Goog-Algorithm%3DGOOG4-RSA-SHA256%26X-Goog-Credential%3Dgcp-kaggle-com%2540kaggle-161607.iam.gserviceaccount.com/20241227/auto/storage/goog4_request%26X-Goog-Date%3D20241227T012455Z%26X-Goog-Expires%3D259200%26X-Goog-SignedHeaders%3Dhost%26X-Goog-Signature%
...
output_path = "/kaggle/input/imasksout/output_masks"

root_dir = "/kaggle/input/recyclable-and-household-waste-classification/images/images"
mask_dir = "/kaggle/input/imasksout/output_masks"
...
train_image_transform = v2.Compose([
    ...
    v2.ToTensor(),  # Converts image to tensor and scales it to [0, 1]
    v2.Normalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5))  # Normalize the image
])
...
# Mask transformations - only resizing
mask_transform = v2.Compose([
    v2.Resize(size=(256, 256)),
    v2.ToTensor(),  # Converts mask to tensor (should remain in [0, 1] range)
])

# Data transformations for each split
data_transforms = {
    "train": (train_image_transform, mask_transform),
    "val": (val_image_transform, mask_transform),
    "test": (test_image_transform, mask_transform),
}
...
class WasteDatasetWithMask(Dataset):
    def __init__(self, root_dir, mask_dir, split='train', image_transform=None, mask_transform=None, dirs_select_from=['default', 'real_world']):
        ...
        for i, class_name in enumerate(self.classes):
            ...
            for subfolder in self.dirs_select_from:
                ...
                if not os.path.exists(subfolder_dir) or not os.path.exists(mask_subfolder_dir):
                    print(f"Skipping {subfolder_dir} or {mask_subfolder_dir} as it does not exist.")
                    continue
                ...
                if split == 'train':
                    ...
                    image_names = image_names[int(0.8 * len(image_names)):int(0.9 * len(image_names))]
                elif split == 'test':
    ...
    def __getitem__(self, index):
        ...
        if self.image_transform:
            image = self.image_transform(image)  # Transform for image (including normalization)
        if self.mask_transform:
            mask = self.mask_transform(mask)  # Transform for mask (only resizing)

        return {"image": image, "mask": mask, "label": label}

train_dataset = WasteDatasetWithMask(
    root_dir="/kaggle/input/recyclable-and-household-waste-classification/images/images",
    mask_dir="/kaggle/input/imasksout/output_masks",
    split="train",
    image_transform=data_transforms["train"][0],  # Image transformation
    mask_transform=data_transforms["train"][1]    # Mask transformation
)

val_dataset = WasteDatasetWithMask(
    root_dir="/kaggle/input/recyclable-and-household-waste-classification/images/images",
    mask_dir="/kaggle/input/imasksout/output_masks",
    split="val",
    image_transform=data_transforms["val"][0],  # Image transformation
    mask_transform=data_transforms["val"][1]    # Mask transformation
)

test_dataset = WasteDatasetWithMask(
    root_dir="/kaggle/input/recyclable-and-household-waste-classification/images/images",
    mask_dir="/kaggle/input/imasksout/output_masks",
    split="test",
    image_transform=data_transforms["test"][0],  # Image transformation
    mask_transform=data_transforms["test"][1]    # Mask transformation
)
...
config = {
    'MODEL_TYPE': 'vit_b',
    'SAM_CHECKPOINT': '/kaggle/input/sam/other/default/1/sam_vit_b.pth',
...
# Define the visualize_batch function for both image and mask
def visualize_batch(batch, classes, dataset_type, batch_size=16):
    fig = plt.figure("{} batch".format(dataset_type), figsize=(8, 8))
    mean = np.array([0.5, 0.5, 0.5])  # Normalization mean
    std = np.array([0.5, 0.5, 0.5])   # Normalization std

    for i in range(batch_size):
        ax = plt.subplot(8, 4, 2 * i + 1)  # Image subplot (odd indices)
        ...
        image = (image * 255).astype("uint8")  # Convert to 0-255 range for display

        # Display the image
        ax.imshow(image)
        ax.set_title(f"Image - {classes[batch['label'][i].item()]}")
        ax.axis("off")

        # Create another subplot for the mask (even indices)
        ax2 = plt.subplot(8, 4, 2 * i + 2)  # Mask subplot (even indices)
        mask = batch["mask"][i].cpu().numpy()
        mask = np.squeeze(mask, axis=0)  # Remove the channel dimension
        mask = (mask * 255).astype("uint8")  # Convert to 0-255 range for display

        ax2.imshow(mask, cmap='gray')  # Display mask in grayscale
        ax2.set_title("Mask")
        ax2.axis("off")
...
# Assuming you have a data loader like this
train_batch = next(iter(data_loaders["train"]))  # Get a batch of data
visualize_batch(train_batch, class_names, "train")  # Visualize the batch
...
# Initialize the model with the number of classes and move it to the device
num_classes = len(class_names)  # Number of classes
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = WasteClassificationModelWithMask(num_classes=num_classes)
...
"""# **Section 6: Model Initialization & Training**"""

model = WasteClassificationModelWithMask(num_classes=len(class_names))  # Pass the number of classes
model = model.to(device)
...
for epoch in range(num_epochs):
    ...
    for phase in ["train", "val"]:
        ...
        for idx, data in enumerate(data_loaders[phase]):
            images = data["image"].to(device)  # Shape: [batch_size, 3, H, W]
            masks = data["mask"].to(device)  # Shape: [batch_size, 1, H, W]

            # Concatenate mask as the 4th channel
            inputs = torch.cat((images, masks), dim=1)  # Shape: [batch_size, 4, H, W]
            labels = data["label"].to(device)  # Load labels
...
# Map the checkpoint to the CPU
checkpoint = torch.load(
    "/kaggle/input/trn/other/default/1/train_loss_best.pt",
...
for idx, data in enumerate(data_loaders["val"]):
    ...
    # Concatenate masks as the fourth channel
    inputs = torch.cat((images, masks), dim=1)  # Shape: [batch_size, 4, H, W]
...
"""# **Section 8: Disposal Recommendations**"""

# Test batch with recommendations
test_batch = next(iter(data_loaders["test"]))
images = test_batch["image"].to(device)  # [batch_size, 3, H, W]
masks = test_batch["mask"].to(device)    # [batch_size, 1, H, W]
labels = test_batch["label"].to(device)  # Labels

# Concatenate images and masks
inputs = torch.cat((images, masks), dim=1)  # [batch_size, 4, H, W]
...
# Visualization
fig = plt.figure("Test Batch with Recommendations", figsize=(16, 16))
mean = np.array([0.5, 0.5, 0.5])  # Mean for normalization
std = np.array([0.5, 0.5, 0.5])   # Std for normalization
...
# Print disposal recommendations in the console
for i in range(len(predictions)):
    predicted_label = class_names[predictions[i]]
    disposal_recommendation = disposal_methods.get(predicted_label, "No recommendation available.")
...
def classify_with_mask(image_path, model, mask_generator, device, transform, class_names, disposal_methods):
    ...
    print(f"Disposal method: {disposal_methods[class_names[predicted_class]]}")

from segment_anything import SamAutomaticMaskGenerator

# Initialize SAM and mask generator
sam = sam_model_registry['vit_b'](checkpoint='/kaggle/input/sam/other/default/1/sam_vit_b.pth')
sam.to(device)

# Create a mask generator
mask_generator = SamAutomaticMaskGenerator(sam)

# Initialize WasteClassificationModelWithMask
model = WasteClassificationModelWithMask(num_classes=len(class_names))
model.load_state_dict(torch.load(
    '/kaggle/input/trn/other/default/1/train_loss_best.pt',
    map_location=torch.device('cpu')  # Map weights to CPU
)['model_state_dict'])
model.to(device)
...
transform = transforms.Compose([
    ...
    transforms.Normalize(mean=[0.5, 0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5, 0.5])  # Normalize 4 channels
])


# Real-time prediction
image_path = "/kaggle/input/recyclable-and-household-waste-classification/images/images/cardboard_packaging/real_world/Image_104.png"
classify_with_mask(image_path, model, mask_generator, device, transform, class_names, disposal_methods)